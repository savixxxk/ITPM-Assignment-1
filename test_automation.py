...
def _fallback_output_path(output_path: str) -> str:
    path = Path(output_path)
    return str(path.with_name(f"{path.stem}.results{path.suffix}"))

def _save_workbook_with_fallback(wb, primary_path: str) -> str:
    try:
        wb.save(primary_path)
        return primary_path
    except PermissionError:
        fallback_path = _fallback_output_path(primary_path)
        wb.save(fallback_path)
        print(f"Primary output is locked; saved results to {fallback_path}")
        ...
        for row_index in range(header_row + 1, int(ws.max_row or 0) + 1):
            ...
            try:
                ...
                if args.save_every and int(args.save_every) > 0 and processed % int(args.save_every) == 0:
                    _save_workbook_with_fallback(wb, args.output)
